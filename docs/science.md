...
**synth-saxs** is built on a robust set of fundamental biophysical equations for simulating Small-Angle X-ray Scattering (SAXS) from atomic coordinates. This document outlines the physical models and their limits of validity.
...
`synth-saxs` provides built-in visualization tools to analyze the overall shape and flexibility of the simulated structure.
...
While `synth-saxs` provides a highly accurate O(N²) calculation, users should be aware of the underlying physical assumptions:
...
3. **Static Representation:** A single PDB file is a static snapshot. Real SAXS data is an ensemble average over all conformational states in solution. For flexible proteins, calculating the profile of a single structure will fail to capture the solution reality. We recommend integrating `synth-saxs` with ensemble generators for these cases.
